...
import { Requests, Webhooks } from '@unhook/db/schema';
...
export class WebhookHandler {
  ...
  private async forwardRequest(record: WebhookRecord): Promise<void> {
    ...
    if (webhook.config.storage.storeResponseBody) {
      const bodySize = responseBody.byteLength;
      if (bodySize <= webhook.config.storage.maxResponseBodySize) {
    ...
    const responseHeaders = webhook.config.storage.storeHeaders
      ? filterHeaders(
          Object.fromEntries(response.headers.entries()),

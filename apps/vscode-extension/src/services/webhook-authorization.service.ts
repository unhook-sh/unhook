...
export class WebhookAuthorizationService implements vscode.Disposable {
  ...
  private _onDidChangeAuthorizationState = new vscode.EventEmitter<WebhookAuthorizationState>();
  readonly onDidChangeAuthorizationState = this._onDidChangeAuthorizationState.event;
  ...
  async handleAuthorizationError(webhookId: string, authStore: AuthStore): Promise<void> {
    ...
    if (authStore.isSignedIn && webhookId) {
      try {
        const pendingRequest = await authStore.api.webhookAccessRequests.checkPendingRequest.query({
          webhookId,
        });
        this.state.hasPendingRequest = !!pendingRequest;
        log('Pending request check', { hasPendingRequest: this.state.hasPendingRequest });
      } catch (err) {
  ...
  async requestAccess(authStore: AuthStore, message?: string): Promise<void> {
    ...
    log('Requesting webhook access', { webhookId: this.state.webhookId, hasMessage: !!message });

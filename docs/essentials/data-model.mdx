...
flowchart TD
    subgraph Tunnel[Tunnel]
        T[id: string<br/>clientId: string<br/>tunnelId: string<br/>port: number<br/>status: string]
    end

    subgraph Event[Event]
        E[id: string<br/>tunnelId: string<br/>originalRequest: RequestPayload<br/>status: string]
    end

    subgraph Request[Request]
        R[id: string<br/>tunnelId: string<br/>eventId: string<br/>request: RequestPayload<br/>status: string]
    end

    Tunnel -->|1:Many| Event
    Event -->|1:Many| Request
    Tunnel -->|1:Many| Request
```
...
<ParamField body="tunnelId" type="string" required>
...
<ParamField body="status" type="string" required>
  Current tunnel status. Can be 'active' or 'inactive'
</ParamField>
...
<ParamField body="config" type="object" required>
  ...
  ```typescript
  {
    ...
    requests: {
      ...
      allowedPaths?: string[];    // Only allow specific paths
      blockedPaths?: string[];    // Block specific paths
...
<Steps>
  <Step title="Webhook Reception">
    ...
    - Event is associated with the target Tunnel
    - Initial status is set to 'pending'
  </Step>
  <Step title="Processing">
    - System creates a Request for delivery attempt
    - Request inherits Event and Tunnel properties
    - System attempts to forward to local environment
  </Step>

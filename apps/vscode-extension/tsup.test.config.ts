...
export default defineConfig({
  bundle: true,
  clean: false,
  entry: ['src/test/integ/**/*.test.ts', 'src/test/integ/utils/test-runner.ts'], // VS Code extensions require CommonJS
  env: {
    ...process.env,
  ...
  noExternal: [
    ...
    /^(?!vscode$|assert$|buffer$|child_process$|crypto$|events$|fs$|http$|https$|net$|os$|path$|process$|readline$|stream$|string_decoder$|url$|util$|zlib$|mocha$)/,
  ],
  onSuccess: async () => {
    console.log('✅ VS Code extension build complete');
  },
  outDir: 'dist/test/integ',
  platform: 'node',
  sourcemap: 'inline',

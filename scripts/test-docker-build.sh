...
# Test CLI build using Docker to simulate CI environment
# This provides a clean, isolated environment similar to GitHub Actions

set -e

echo "🐳 Testing CLI build in Docker environment"
...
docker build -f Dockerfile.test -t unhook-cli-test .

# Run the test
echo "🚀 Running CLI build test in Docker..."
docker run --rm unhook-cli-test

# Cleanup
rm Dockerfile.test

echo "🎉 Docker-based CI test completed successfully!"